...
void NNbarRun::RecordEvent(const G4Event* event)
{
    ...
    if (HCE) {
        ...
        if (CarbonHits) {
            ...
            for (G4int h=0; h<hitCount; h++) {
                ...
                Carbon_outFile << event->GetEventID() << "," << trID << "," << parentID << "," << name << "," << proc << "," << step_info << "," << origin_volume
        ...
        if (SiliconHits) {
            ...
            for (G4int h=0; h<hitCount; h++) {
                ...
                G4double u = ((*SiliconHits)[h]) -> GetPX();
                G4double v = ((*SiliconHits)[h]) -> GetPY();
                G4double w = ((*SiliconHits)[h]) -> GetPZ();
                ...
                Silicon_outFile << event->GetEventID() <<"," << i << "," << trID << "," << parentID << "," << name << "," << proc << "," << step_info << "," << origin_volume
                <<","<< x <<","<<y<<","<<z<<","<< u<<"," << v << "," << w <<","<<
	    ...
	    hitCount = TubeHits->entries();
            for (G4int h=0; h<hitCount; h++) {
                ...
                G4double u = ((*TubeHits)[h]) -> GetPX();
                G4double v = ((*TubeHits)[h]) -> GetPY();
                G4double w = ((*TubeHits)[h]) -> GetPZ();
                ...
                Tube_outFile << event->GetEventID() << "," << trID << "," << parentID << "," << name << "," << proc << "," << current_vol << "," << step_info << "," << origin_volume
                <<"," << x <<","<<y<<","<<z<<","<< u<<"," << v << "," << w <<","
        ...
        if (TPCHits) {
            ...
            for (G4int h=0; h<hitCount; h++) {
                ...
                //SD_outFile << local_event_number<<","<<trID<<","<<parentID<<","<<name<<","<<proc<<","<< "TPC"<<","<< 0 <<","<< 0 <<","<<i<<","<<x<<","<<y<<","<<z<<","<<
                //time<<","<<kinEn<<","<<eDep<<","<< photons <<G4endl;
                //if (electrons>0){
                    if (proc !="eIoni" && proc!= "hIoni"&& proc!= "muIoni") {// dont want to see the electrons ... 
                    TPC_outFile << event->GetEventID() << "," << module_ID << "," << i << "," << trID << "," << parentID << "," << name << ","<< proc <<"," << step_info <<","<< origin_volume <<","  << x <<","<<y<<","<<z<<","
        ...
        if (ScintHits) {
           ...
           for (G4int h=0; h<hitCount; h++) { 
               ...
               if (name != "opticalphoton"){
                    ...
                    G4String current_vol = ((*ScintHits)[h]) -> GetVolName();
                    // writing output

                    //if (scint_photons_per_hit>0){
                    Scint_layer_outFile << event->GetEventID() <<","<<trID<<","<<parentID<<","<<name<<","<<proc<<","<< current_vol << ","
        ...
        if(AbsHits) {
            for (G4int h=0; h<AbsHits->entries(); h++) {
                ...
                if (name != "opticalphoton"){
                    ...
                    y = ((*AbsHits)[h]) -> GetPosY();
                    z = ((*AbsHits)[h]) -> GetPosZ();
                    G4String current_vol = ((*AbsHits)[h]) -> GetVolName();
                    int step_info = ((*AbsHits)[h]) -> GetStepInfo();
                    G4String origin_volume = ((*AbsHits)[h]) -> GetOriginVolName();

                    //if (photons_cerenkov>0){
                    Abs_outFile << event->GetEventID() <<","<<trID << ","<<parentID<<","<<name<<","<<proc<<","<< current_vol << ","
                    << origin_volume << "," << step_info << ","
                    <<i<<","
        ...
        //                 Shield_outFile << local_event_number << "," << trID << "," << parentID << "," << name << "," << proc << "," << vol_name << "," << x <<","<<y<<","<<z<<","
    ...
    if ( ( printModulo > 0 ) && ( eventID % printModulo == 0 ) ) {G4cout << "---> End of event: " << local_event_number << " local event ID: "<< eventID << G4endl;}
}
