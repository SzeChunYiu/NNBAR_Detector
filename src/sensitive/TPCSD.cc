...
#if WITH_GARFIELD_GPU
        // Add ionization data for GPU drift simulation
        if (electrons > 0) {
            nnbar::TPCDriftManager::Instance()->AddIonization(
                static_cast<float>(x),    // mm
                static_cast<float>(y),    // mm
                static_cast<float>(z),    // mm
                static_cast<float>(time), // ns
                electrons,
                TPC_index,
                k,
                trackID
