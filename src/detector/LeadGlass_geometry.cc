// ============================================================================
// Lead Glass Calorimeter Geometry Construction
// Implements Schott SF5 lead glass blocks with PMT readout
// Optical surfaces: reflective sides, absorbing bottom, transparent PMT coupling
// ============================================================================
...
std::string filename_data_lead_glass_pos = "./data/lead_glass_position/lead_glass_position.csv";
std::string filename_data_lead_glass_pos_fb = "./data/lead_glass_position/lead_glass_position_fb.csv";
...
void import_lead_glass_pos(std::string file_name, std::vector<std::vector<double> >& data) {
	...
	if (!init_file.is_open()) {
		...
		msg << "Unable to open lead-glass position file: " << file_name;
		G4Exception("import_lead_glass_pos", "LeadGlassMissingPositionFile", FatalException, msg);
		return;
	}
	...
	while (getline(init_file, line)) {
		...
		while (std::getline(iss, token, ',')) {
			values.push_back(boost::lexical_cast<double>(token.c_str()));
		}
	...
	if (data.empty()) {
		G4ExceptionDescription msg;
		msg << "Lead-glass position file is empty: " << file_name;
		G4Exception("import_lead_glass_pos", "LeadGlassEmptyPositionFile", FatalException, msg);
		return;
	}
...
void LeadGlass::DefineMaterials()
{
  ...
  // Only build elements that are actually used
  ...
  ////////////////////////////////////////////////////
    ...
    LeadGlass_MPT->AddProperty("RINDEX", PhotonEnergy, refractiveIndex, nEntries);
...
std::vector<G4LogicalVolume*> LeadGlass::Construct_Volumes(G4LogicalVolume* mother)
{
  ...
  ////////////////////////////////////////////////////////////
    ...
    auto LeadGlass_module_LV = new G4LogicalVolume(LeadGlass_module_S,defaultMaterial,"LeadGlass_module_LV");
        ...
        new G4PVPlacement(0,G4ThreeVector(0., -(lead_glass_y+PMT_thickness)/2.+(lead_glass_y)/2., 0.),leadglassLV,"LeadGlassPV",LeadGlass_module_LV,false,0,false);
        ...
        new G4PVPlacement(0,G4ThreeVector(0.,0.,0.),coatingLV,"LeadGlass_CoatingPV",LeadGlass_module_LV,false,0,false);
        ...
        new G4PVPlacement(0,G4ThreeVector(0., -(lead_glass_y+PMT_thickness)/2.+lead_glass_y+PMT_thickness/2., 0.),PMTLV,"PMTPV",LeadGlass_module_LV,false,0,false);
  ...
  //////////////////////////////////////////////////////////////////////
    ...
    G4double rot_angle = 90.0*deg;
        ...
        for (size_t i = 0; i < 4; i++){
          ...
          for (size_t j = 0 ; j < data_lead_glass_pos.size();j++){temp_rot_array_dir.push_back(new G4RotationMatrix);}
        ...
        for (size_t i = 0 ; i<4;i++){
          for (size_t j = 0 ; j <data_lead_glass_pos.size();j++){ // data_lead_glass_pos.size()
            ...
            int32_t copyNumber = i*data_lead_glass_pos.size()+j;
            lg_PV_array[i].push_back(new G4PVPlacement(rot_array_dir[i][j],G4ThreeVector(lead_x, lead_y,data_lead_glass_pos[j][2]*cm) ,LeadGlass_module_LV,"LeadGlassPV",mother,false,copyNumber,false));

            // Register with GeometryManager
            geoMgr.RegisterLeadGlassBlock(copyNumber,
                                          lead_x/mm, lead_y/mm, data_lead_glass_pos[j][2]*cm/mm,
                                          data_lead_glass_pos[j][3], data_lead_glass_pos[j][5],
                                          i);  // surfaceIndex = i (0=top, 1=right, 2=bottom, 3=left)
          }
        }
    ...
    int lead_index_fb = 4*data_lead_glass_pos.size();
        ...
        for (size_t i = 0 ; i < data_lead_glass_pos_fb.size();i++){rot_array_dir111.push_back(new G4RotationMatrix);}
        for (size_t i = 0 ; i < data_lead_glass_pos_fb.size();i++){rot_array_dir211.push_back(new G4RotationMatrix);}

        // Front lead glass (surfaceIndex = 4)
        for (size_t i = 0 ; i < data_lead_glass_pos_fb.size();i++){
              rot_array_dir111[i]-> rotateX(-data_lead_glass_pos_fb[i][3]*deg+90.*deg); rot_array_dir111[i]-> rotateZ(data_lead_glass_pos_fb[i][5]*deg);
              G4double front_x = data_lead_glass_pos_fb[i][0]*cm;
              G4double front_y = data_lead_glass_pos_fb[i][2]*cm;
              G4double front_z = -(data_lead_glass_pos_fb[i][1]*cm);
              int32_t copyNum = lead_index_fb+i;
              new G4PVPlacement(rot_array_dir111[i],
                  G4ThreeVector(front_x, front_y, front_z),
                  LeadGlass_module_LV,"LeadGlassPV",mother,false,copyNum,false);
              // Register front lead glass
              geoMgr.RegisterLeadGlassBlock(copyNum,
                                            front_x/mm, front_y/mm, front_z/mm,
                                            data_lead_glass_pos_fb[i][3], data_lead_glass_pos_fb[i][5],
                                            4);  // surfaceIndex = 4 (front)
        }

    lead_index_fb = 4*data_lead_glass_pos.size() + data_lead_glass_pos_fb.size();

        // Back lead glass (surfaceIndex = 5)
        for (size_t i = 0 ; i < data_lead_glass_pos_fb.size();i++){
              rot_array_dir211[i]-> rotateX(data_lead_glass_pos_fb[i][3]*deg-90.*deg); rot_array_dir211[i]-> rotateZ(data_lead_glass_pos_fb[i][5]*deg);
              G4double back_x = data_lead_glass_pos_fb[i][0]*cm;
              G4double back_y = data_lead_glass_pos_fb[i][2]*cm;
              G4double back_z = data_lead_glass_pos_fb[i][1]*cm;
              int32_t copyNum = lead_index_fb+i;
              new G4PVPlacement(rot_array_dir211[i],
                  G4ThreeVector(back_x, back_y, back_z),
                  LeadGlass_module_LV,"LeadGlassPV",mother,false,copyNum,false);
              // Register back lead glass
              geoMgr.RegisterLeadGlassBlock(copyNum,
                                            back_x/mm, back_y/mm, back_z/mm,
                                            data_lead_glass_pos_fb[i][3], data_lead_glass_pos_fb[i][5],
                                            5);  // surfaceIndex = 5 (back)
        }
        
  // ============================================================================
  // Optical Surface Configuration for Lead Glass Blocks
  // ============================================================================
  // - Sides (coating): 95% reflective for internal light collection
  // - Bottom (facing detector): Absorbing (handled by world boundary)
  // - Top (PMT): Transparent with ~25% quantum efficiency
  // ============================================================================

  // Side surfaces - reflective coating (AlMgF2)
  G4OpticalSurface* op_coating = new G4OpticalSurface("lg_side_coating");
  op_coating->SetType(dielectric_metal);
  op_coating->SetFinish(polished);
  op_coating->SetModel(unified);

  G4double pp_coating[] = { 2.0 * eV, 3.5 * eV };
  const G4int num_coating = sizeof(pp_coating) / sizeof(G4double);
  G4double reflectivity_coating[] = { 0.95, 0.95 };  // 95% reflective sides
  G4double efficiency_coating[] = { 0.0, 0.0 };       // No detection on sides
  ...
  new G4LogicalSkinSurface("lg_coating_surface", coatingLV, op_coating);

  // PMT surface - transparent with quantum efficiency for photon detection
  G4OpticalSurface* op_pmt = new G4OpticalSurface("lg_pmt_surface");
  op_pmt->SetType(dielectric_metal);
  op_pmt->SetFinish(polished);
  op_pmt->SetModel(unified);

  G4double pp_pmt[] = { 2.0 * eV, 3.5 * eV };
  const G4int num_pmt = sizeof(pp_pmt) / sizeof(G4double);
  G4double reflectivity_pmt[] = { 0.0, 0.0 };     // No reflection - all transmitted or detected
  G4double efficiency_pmt[] = { 0.25, 0.25 };     // ~25% PMT quantum efficiency

  G4MaterialPropertiesTable* PMTProperty = new G4MaterialPropertiesTable();
  PMTProperty->AddProperty("REFLECTIVITY", pp_pmt, reflectivity_pmt, num_pmt);
  PMTProperty->AddProperty("EFFICIENCY", pp_pmt, efficiency_pmt, num_pmt);
  op_pmt->SetMaterialPropertiesTable(PMTProperty);

  new G4LogicalSkinSurface("lg_pmt_surface", PMTLV, op_pmt);

  G4cout << "LeadGlass: Optical surfaces configured - sides 95% reflective, PMT 25% QE" << G4endl;
