...
//#include "sensitive/Cosmic_ShieldingSD.hh"
...
void CosmicShielding::DefineMaterials()
{
  ...
  // Lead material
  new G4Material("Lead", z=82., a= 207.2*g/mole, density = 11.29*g/cm3);

  // PE_B4C_concrete - normalized mass fractions (must sum to 1.0)
  G4Material* PE_B4C_concrete = new G4Material("PE_B4C_concrete", density = 1.97*g/cm3, 15);
  PE_B4C_concrete->AddElement(elO,  0.4343);  // Oxygen
  PE_B4C_concrete->AddElement(elCa, 0.0759);  // Calcium
  PE_B4C_concrete->AddElement(elSi, 0.2678);  // Silicon
  PE_B4C_concrete->AddElement(elAl, 0.0221);  // Aluminum
  PE_B4C_concrete->AddElement(elFe, 0.00789); // Iron
  PE_B4C_concrete->AddElement(elMg, 0.00184); // Magnesium
  PE_B4C_concrete->AddElement(elNa, 0.00578); // Sodium
  PE_B4C_concrete->AddElement(elK,  0.01179); // Potassium
  PE_B4C_concrete->AddElement(elS,  0.00260); // Sulfur
  PE_B4C_concrete->AddElement(elCl, 0.000333);// Chlorine
  PE_B4C_concrete->AddElement(elH,  0.02227); // Hydrogen
  PE_B4C_concrete->AddElement(elTi, 0.00487); // Titanium
  PE_B4C_concrete->AddElement(elP,  0.00244); // Phosphorus
  PE_B4C_concrete->AddElement(elC,  0.0842);  // Carbon
  PE_B4C_concrete->AddElement(elB,  0.0562);  // Boron
  ...
  // Check if StainlessSteel already defined (may be in beampipe_geometry)
...
std::vector<G4LogicalVolume*> CosmicShielding::Construct_Volumes(G4LogicalVolume* mother)
{
  ...
  ///////////////////////////////////////////////////////////////////////
    ...
    new G4PVPlacement(0,G4ThreeVector(0.,LeadShield_top_y,0.),LeadShield_top_LV,"LeadShield",mother,false,0,false);
    new G4PVPlacement(0,G4ThreeVector(0.,-LeadShield_top_y,0.),LeadShield_top_LV,"LeadShield",mother,false,0,false);
    ...
    new G4PVPlacement(0,G4ThreeVector(0.,SteelShield_top_y,0.),SteelShield_top_LV,"SteelShield",mother,false,0,false);
    new G4PVPlacement(0,G4ThreeVector(0.,-SteelShield_top_y,0.),SteelShield_top_LV,"SteelShield",mother,false,0,false);
    ...
    new G4PVPlacement(0,G4ThreeVector(LeadShield_side_x,0.,0.),LeadShield_side_LV,"LeadShield_side",mother,false,0,false);
    new G4PVPlacement(0,G4ThreeVector(-(LeadShield_side_x),0.,0.),LeadShield_side_LV,"LeadShield_side",mother,false,0,false);
    ...
    new G4PVPlacement(0,G4ThreeVector(SteelShield_side_x,0.,0.),SteelShield_side_LV,"SteelShield_side",mother,false,0,false);
    new G4PVPlacement(0,G4ThreeVector(-(SteelShield_side_x),0.,0.),SteelShield_side_LV,"SteelShield_side",mother,false,0,false);
    ...
    // ===== Front back surfaces
      ...
      // Need to make a hole on the surface
        ...
        new G4PVPlacement(0,G4ThreeVector(0.,0.,LeadShield_fb_z),LeadShield_fb_LV,"LeadShield_front",mother,false,0,false);
      ...
      // Need to make a hole on the surface
        ...
        new G4PVPlacement(0,G4ThreeVector(0.,0.,SteelShield_fb_z),SteelShield_fb_LV,"SteelShield_front",mother,false,0,false);
