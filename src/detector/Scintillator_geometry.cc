// ============================================================================
// Scintillator Geometry Construction
// Implements BC-408 plastic scintillator bars with proper optical surfaces
// ============================================================================
...
void Scintillator::DefineMaterials()
{
  ...
  // Note: In Geant4 11.x, spline interpolation is now the default
  ...
  scintMPT->AddProperty("SCINTILLATIONCOMPONENT1", ScintPhotonEnergy, scintilFast, nEntries2);
  scintMPT->AddProperty("SCINTILLATIONCOMPONENT2", ScintPhotonEnergy, scintilSlow, nEntries2);

  // BC-408 scintillation yield: ~10,000 photons/MeV
  // Conditional on WITH_SCINTILLATION to allow fast simulation without optical photons
#if WITH_SCINTILLATION
  scintMPT->AddConstProperty("SCINTILLATIONYIELD", 10000./ MeV);
  G4cout << "Scintillator: Optical photon generation ENABLED (10000 photons/MeV)" << G4endl;
#else
  scintMPT->AddConstProperty("SCINTILLATIONYIELD", 0./ MeV);
  G4cout << "Scintillator: Optical photon generation DISABLED (fast mode)" << G4endl;
#endif

  scintMPT->AddConstProperty("RESOLUTIONSCALE", 1.0);
  // Geant4 11.x renamed scintillation time constants
  scintMPT->AddConstProperty("SCINTILLATIONTIMECONSTANT1", 0.9*ns);  // BC-408 fast decay
  scintMPT->AddConstProperty("SCINTILLATIONTIMECONSTANT2", 2.1*ns);  // BC-408 slow decay
  scintMPT->AddConstProperty("SCINTILLATIONYIELD1", 1.0);  // Fraction for fast component
  scintMPT->AddConstProperty("SCINTILLATIONYIELD2", 0.0);  // Fraction for slow component
...
std::vector<G4LogicalVolume*> Scintillator::Construct_Volumes(G4LogicalVolume* mother)
{
  ...
  //// ------ Scintillator bars
      ...
      for (int i=0; i<no_of_Layers; i++){
        if (i%2==0){new G4PVPlacement(0, G4ThreeVector(0.,-scint_module_y/2.+(2*i+1)/2.*scint_bar_y,0.),scint_layerH_LV,"Scint_layerPV",scint_moduleLV,false,i,false);}
        else{new G4PVPlacement(0, G4ThreeVector(0.,-scint_module_y/2.+(2*i+1)/2.*scint_bar_y,0.),scint_layerV_LV,"Scint_layerPV",scint_moduleLV,false,i,false);}
      }

      // placing the bars
      for (int i=0; i<n_bar_x; i++){
        new G4PVPlacement(0,G4ThreeVector(-scint_module_x/2.+(2*i+1)/2.*scint_barH_x,0.,0.),scint_barH_LV,"Scint_barPV_H",scint_layerH_LV,false,i,false);
        new G4PVPlacement(0,G4ThreeVector(0.,0.,-scint_module_z/2.+(2*i+1)/2.*scint_barV_z),scint_barV_LV,"Scint_barPV_V",scint_layerV_LV,false,i,false);
      }
      ...
      // Get GeometryManager instance for registration
      auto& geoMgr = nnbar::GeometryManager::Instance();

      for (size_t i=0; i<scint_module_pos_vector_horizontal.size(); i++){
        ...
        int32_t copyNum1 = i;
        int32_t copyNum2 = scint_module_pos_vector_horizontal.size()+i;
        int32_t copyNum3 = 2*scint_module_pos_vector_horizontal.size()+i;
        int32_t copyNum4 = 3*scint_module_pos_vector_horizontal.size()+i;

        new G4PVPlacement(zRot1,scint_mod_pos1,scint_moduleLV,"ScintPV",mother,false,copyNum1,false);
        new G4PVPlacement(zRot2,scint_mod_pos2,scint_moduleLV,"ScintPV",mother,false,copyNum2,false);
        new G4PVPlacement(zRot3,scint_mod_pos3,scint_moduleLV,"ScintPV",mother,false,copyNum3,false);
        new G4PVPlacement(zRot4,scint_mod_pos4,scint_moduleLV,"ScintPV",mother,false,copyNum4,false);

        // Register scintillator modules with GeometryManager
        geoMgr.RegisterScintillatorModule(copyNum1, scint_mod_pos1[0]/mm, scint_mod_pos1[1]/mm, scint_mod_pos1[2]/mm,
                                          0, 0.0);     // Top surface, 0 deg rotation
        geoMgr.RegisterScintillatorModule(copyNum2, scint_mod_pos2[0]/mm, scint_mod_pos2[1]/mm, scint_mod_pos2[2]/mm,
                                          3, 270.0);   // Left surface, 270 deg rotation
        geoMgr.RegisterScintillatorModule(copyNum3, scint_mod_pos3[0]/mm, scint_mod_pos3[1]/mm, scint_mod_pos3[2]/mm,
                                          2, 180.0);   // Bottom surface, 180 deg rotation
        geoMgr.RegisterScintillatorModule(copyNum4, scint_mod_pos4[0]/mm, scint_mod_pos4[1]/mm, scint_mod_pos4[2]/mm,
                                          1, 90.0);    // Right surface, 90 deg rotation

        scintillator_module_pos_outFile << copyNum1 << "," << scint_mod_pos1[0]/cm << "," << scint_mod_pos1[1]/cm << "," << scint_mod_pos1[2]/cm << G4endl;
        scintillator_module_pos_outFile << copyNum2 << "," << scint_mod_pos2[0]/cm << "," << scint_mod_pos2[1]/cm << "," << scint_mod_pos2[2]/cm << G4endl;
        scintillator_module_pos_outFile << copyNum3 << "," << scint_mod_pos3[0]/cm << "," << scint_mod_pos3[1]/cm << "," << scint_mod_pos3[2]/cm << G4endl;
        scintillator_module_pos_outFile << copyNum4 << "," << scint_mod_pos4[0]/cm << "," << scint_mod_pos4[1]/cm << "," << scint_mod_pos4[2]/cm << G4endl;
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////
      ...
      auto scint_module_fb_LV = new G4LogicalVolume(scint_module_fb_S,defaultMaterial,"Scint_fb_LV1");
          ...
          auto scint_layer_fb2_LV = new G4LogicalVolume(scint_layer_fb2_S,defaultMaterial,"Scint_bar_LV12");
              ...
              for (int i=0; i<no_of_Layers; i++){ //10
                if (i%2==0){new G4PVPlacement(0, G4ThreeVector(0.,0.,-scint_module_fb_z/2.+(2.*i+1.)/2.*scint_bar_fb_z),scint_layer_fb1_LV,"Scint_FB_layerPV",scint_module_fb_LV,false,i,false);}
                else{new G4PVPlacement(0, G4ThreeVector(0.,0.,-scint_module_fb_z/2.+(2.*i+1.)/2.*scint_bar_fb_z),scint_layer_fb2_LV,"Scint_FB_layerPV",scint_module_fb_LV,false,i,false);}
              }

              // placing the staves 
              for (int i=0; i<n_bar_x_fb; i++){
                // --- horizontal ones for odd number layers
                new G4PVPlacement(0,G4ThreeVector(0.,-scint_module_fb_y/2.+(2.0*i+1.0)/2.*scint_bar_yH_fb1,0.),scint_bar_fb1H_LV,"Scint_FB_barPV_H",scint_layer_fb1_LV,false,i,false);
                // --- vertical ones for even number layers
                new G4PVPlacement(0,G4ThreeVector(-scint_module_fb_x/2.+(2.0*i+1.0)/2.*scint_bar_xV_fb1,0.,0.),scint_bar_fb1V_LV,"Scint_FB_barPV_V",scint_layer_fb2_LV,false,i,false);
              }
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (size_t i=0; i<scint_fb_group_1_pos_array.size(); i++){
      ...
      new G4PVPlacement(scint_fb_front_rot,scint_mod_pos_fb1,scint_module_fb_LV,"Scint_FB_PV",mother,false,scint_fb_1_index_1,false);
      new G4PVPlacement(scint_fb_front_rot,scint_mod_pos_fb2,scint_module_fb_LV,"Scint_FB_PV",mother,false,scint_fb_1_index_2,false);
      new G4PVPlacement(scint_fb_back_rot,scint_mod_pos_fb3,scint_module_fb_LV,"Scint_FB_PV",mother,false,scint_fb_1_index_3,false);
      new G4PVPlacement(scint_fb_back_rot,scint_mod_pos_fb4,scint_module_fb_LV,"Scint_FB_PV",mother,false,scint_fb_1_index_4,false);

      // Register front/back scintillator modules (group 1)
      geoMgr.RegisterScintillatorModule(scint_fb_1_index_1, scint_mod_pos_fb1[0]/mm, scint_mod_pos_fb1[1]/mm, scint_mod_pos_fb1[2]/mm,
                                        4, 0.0);    // Front surface
      geoMgr.RegisterScintillatorModule(scint_fb_1_index_2, scint_mod_pos_fb2[0]/mm, scint_mod_pos_fb2[1]/mm, scint_mod_pos_fb2[2]/mm,
                                        4, 0.0);    // Front surface
      geoMgr.RegisterScintillatorModule(scint_fb_1_index_3, scint_mod_pos_fb3[0]/mm, scint_mod_pos_fb3[1]/mm, scint_mod_pos_fb3[2]/mm,
                                        5, 180.0);  // Back surface
      geoMgr.RegisterScintillatorModule(scint_fb_1_index_4, scint_mod_pos_fb4[0]/mm, scint_mod_pos_fb4[1]/mm, scint_mod_pos_fb4[2]/mm,
                                        5, 180.0);  // Back surface
    }
  ...
  ////////////Scintillator fb group 2 Vertical
    ...
    for (size_t i=0; i<scint_fb_group_2_pos_array.size(); i++){
      ...
      new G4PVPlacement(scint_fb_front_rot,scint_mod_pos_fb1,scint_module_fb_LV,"Scint_FB_PV",mother,false,scint_fb_2_index_1,false);
      new G4PVPlacement(scint_fb_front_rot,scint_mod_pos_fb2,scint_module_fb_LV,"Scint_FB_PV",mother,false,scint_fb_2_index_2,false);
      new G4PVPlacement(scint_fb_back_rot,scint_mod_pos_fb3,scint_module_fb_LV,"Scint_FB_PV",mother,false,scint_fb_2_index_3,false);
      new G4PVPlacement(scint_fb_back_rot,scint_mod_pos_fb4,scint_module_fb_LV,"Scint_FB_PV",mother,false,scint_fb_2_index_4,false);

      // Register front/back scintillator modules (group 2)
      geoMgr.RegisterScintillatorModule(scint_fb_2_index_1, scint_mod_pos_fb1[0]/mm, scint_mod_pos_fb1[1]/mm, scint_mod_pos_fb1[2]/mm,
                                        4, 0.0);    // Front surface
      geoMgr.RegisterScintillatorModule(scint_fb_2_index_2, scint_mod_pos_fb2[0]/mm, scint_mod_pos_fb2[1]/mm, scint_mod_pos_fb2[2]/mm,
                                        4, 0.0);    // Front surface
      geoMgr.RegisterScintillatorModule(scint_fb_2_index_3, scint_mod_pos_fb3[0]/mm, scint_mod_pos_fb3[1]/mm, scint_mod_pos_fb3[2]/mm,
                                        5, 180.0);  // Back surface
      geoMgr.RegisterScintillatorModule(scint_fb_2_index_4, scint_mod_pos_fb4[0]/mm, scint_mod_pos_fb4[1]/mm, scint_mod_pos_fb4[2]/mm,
                                        5, 180.0);  // Back surface
    }

  scintillator_module_pos_outFile.close();
  // ============================================================================
  // Optical Surface Configuration for Scintillator Bars
  // ============================================================================
  // Scintillator bars are wrapped with reflective material (Tyvek-like, ~95%)
  // to guide light internally toward the ends where PMTs are coupled.
  // Using skin surface applies to all faces - in reality ends would be
  // transparent for PMT coupling, but this is a good approximation.
  // ============================================================================

    G4OpticalSurface* op_scintillator = new G4OpticalSurface("scintillator_wrap_surface");
    op_scintillator->SetType(dielectric_metal);
    op_scintillator->SetFinish(polished);
    op_scintillator->SetModel(unified);

    // Reflective wrapping (Tyvek-like) for light guiding
    // 95% reflectivity typical for Tyvek wrapping
    G4double pp_scintillator[] = { 2.0 * eV, 3.5 * eV };
    const G4int num_scintillator = sizeof(pp_scintillator) / sizeof(G4double);
    G4double reflectivity_scintillator[] = { 0.95, 0.95 };  // 95% reflective wrapping
    G4double efficiency_scintillator[] = { 0.0, 0.0 };      // No detection on wrapping
    ...
    // Apply reflective wrapping to all scintillator bar surfaces
    new G4LogicalSkinSurface("scint_wrap_H", scint_barH_LV, op_scintillator);
    new G4LogicalSkinSurface("scint_wrap_V", scint_barV_LV, op_scintillator);
    new G4LogicalSkinSurface("scint_wrap_fb1V", scint_bar_fb1V_LV, op_scintillator);
    new G4LogicalSkinSurface("scint_wrap_fb1H", scint_bar_fb1H_LV, op_scintillator);

    G4cout << "Scintillator: Reflective wrapping applied (95% reflectivity)" << G4endl;
